/*Reward the users who have been around the longest
...
/*Day of the week do most users register on? To schedule an ad campaign */
SELECT 
...
/*Target the inactive users with an email campaign
...
/*New contest to see who can get the most likes on a single photo */
SELECT 
    users.username,
    photos.id,
    photos.image_url,
...
FROM
    ...
    HAVING total_likes_by_user = (SELECT 
            COUNT(*)
        FROM
            photos)) AS total_number_users_likes_every_photos) AS tableB;
